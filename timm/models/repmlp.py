...
def _cfg(url='', **kwargs):
    return {
        ...
        'first_conv': 'conv_embedding.conv1', 'classifier': 'fc',
        **kwargs
    }
...
def _create_repmlp(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for MLP models.')

    model = build_model_with_cfg(
        RepMLPNet, variant, pretrained,
...
@register_model
def repmlp_224(pretrained=False, deploy=False, **kwargs):
    model_kwargs = dict(
        channels=(96, 192, 384, 768), hs=(56,28,14,7), ws=(56,28,14,7),
                      num_blocks=(2,2,12,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,32,128),
                     deploy=deploy, **kwargs)
    return _create_repmlp('repmlp_224', pretrained=pretrained, **model_kwargs)


@register_model
def repmlp_256(pretrained=False, deploy=False, **kwargs):
    model_kwargs = dict(channels=(96, 192, 384, 768), hs=(64,32,16,8), ws=(64,32,16,8),
                      num_blocks=(2,2,12,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,32,128),
                      deploy=deploy, **kwargs)
    return _create_repmlp('repmlp_256', pretrained=pretrained, **model_kwargs)
